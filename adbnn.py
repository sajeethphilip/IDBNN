...
#----------------------
    ...
    def _select_samples_from_failed_classes(self, test_predictions, y_test, test_indices):
        """Optimized version with bounds checking"""
        config = self.config.get('active_learning', {})
        strong_margin = config.get('strong_margin_threshold', 0.6)
        marginal_margin = config.get('marginal_margin_threshold', 0.6)
        min_divergence = config.get('min_divergence', 0.1)
        max_class_percent = config.get('max_class_addition_percent', 5)

        # Convert to tensors with device management
        ...
        # Find misclassified samples with bounds checking
        ...
        clamped_preds = torch.clamp(test_preds[mis_mask], 0, num_classes-1)

        # Safely compute prediction probabilities
        pred_probs = posteriors[torch.arange(len(posteriors)), clamped_preds]
        ...
        for class_id in torch.unique(mis_y):
            ...
            # Class-specific data with validation
            class_X = mis_X[class_mask]
            class_indices = mis_indices[class_mask]
            class_margins = margins[class_mask]

            # Split into strong/marginal errors
            strong_mask = class_margins >= strong_margin
            marginal_mask = class_margins <= marginal_margin

            # Process both error types
            for mask, select_top in [(strong_mask, True), (marginal_mask, False)]:
                if not mask.any():
                    continue

                # Get candidate samples
                candidates_X = class_X[mask]
                candidates_idx = class_indices[mask]

                if len(candidates_idx) == 0:
                    continue

                # Cluster selection with divergence check
                clusters = []
                visited = torch.zeros(len(candidates_X), dtype=torch.bool, device=self.device)

                # Compute pairwise divergences
                div_matrix = self._compute_sample_divergence(candidates_X, self.feature_pairs)

                for i in range(len(candidates_X)):
                    if not visited[i]:
                        # Find similar samples
                        cluster_mask = div_matrix[i] < min_divergence
                        clusters.append(candidates_idx[i].item())
                        visited |= cluster_mask

                # Select top representatives
                selected_clusters = clusters[:5]  # Max 5 per group
                selected.extend(selected_clusters)

                # Apply class-level limits
                class_total = (y_test == class_id).sum().item()
                max_samples = max(2, int(class_total * max_class_percent / 100))
                selected = selected[:max_samples]

        # Global safety cap
        return selected[:int(len(test_indices) * 0.02)]

    def _select_divergent_samples(self, X, indices, min_div):
        """Cluster-based selection from original implementation"""
        if len(X) < 2:
            return indices.tolist()

        try:
            # Compute pairwise divergences
            div_matrix = self._compute_sample_divergence(X, self.feature_pairs)

            # Cluster selection
            clusters = []
            visited = torch.zeros(len(X), dtype=torch.bool, device=X.device)
            for i in range(len(X)):
                if not visited[i]:
                    cluster_mask = div_matrix[i] < min_div
                    clusters.append(indices[i].item())
                    visited |= cluster_mask.to(X.device)

            return clusters[:5]  # Return max 5 cluster representatives

        except Exception as e:
            print(f"Divergence clustering failed: {str(e)}")
            return indices[:5].tolist()
