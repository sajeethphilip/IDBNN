...
# Added  ArchitectureController class to automate architecture as per data complexity April 13, 01:14 am
...
class ModelFactory:
    ...
    def create_model(config: Dict) -> nn.Module:
        ...
        arch_controller = ArchitectureController(config)
        ...
        if image_type == 'astronomical':
            ...
            if config['model'].get('encoder_type', 'autoenc') == 'autoenc':
        ...
        # Analyze dataset and adjust model complexity
        if 'train_loader' in config:
            arch_controller.analyze_dataset(config['train_loader'])
            arch_controller.determine_complexity()
...
def train_model(model: nn.Module, train_loader: DataLoader,
    ...
    # Add dataset analysis before training
    if not hasattr(model, 'complexity_factor'):
        arch_controller = ArchitectureController(config)
