...
class PredictionManager:
    ...
    def _load_model(self) -> nn.Module:
        ...
        if 'model_states' in checkpoint:
            # Search through all available phases (using actual keys from checkpoint)
            for phase in checkpoint['model_states'].keys():
                    ...
                    for key in ['best', 'current']:
                        if key in phase_data and isinstance(phase_data[key], dict):
                            if 'state_dict' in phase_data[key]:
                                ...
                                logger.info(f"Loaded state from {phase}/{key}")
                                break
                    if state_dict is not None:
                        break

        # If still not found, try direct checkpoint format
        ...
        if state_dict is None:
            logger.error("Could not find state_dict in checkpoint. Available keys:")
            logger.error(f"Top level: {list(checkpoint.keys())}")
            if 'model_states' in checkpoint:
                logger.error("Model states contains:")
                for phase, data in checkpoint['model_states'].items():
                    logger.error(f"  {phase}: {list(data.keys()) if isinstance(data, dict) else type(data)}")
