...
class PredictionManager:
    ...
    def _load_model(self) -> nn.Module:
        """Load model with dimension verification - FIXED for simple checkpoints"""
        ...
        # Determine if this is a unified checkpoint or simple checkpoint
        if isinstance(loaded, dict) and 'model_states' in loaded:
            ...
            best_state = checkpoint['model_states'][state_key]['best']
            if best_state is None:
                best_state = checkpoint['model_states'][state_key]['current']

            state_dict = best_state['state_dict']

            # Extract metadata
            feature_dims = best_state.get('feature_dims', self.actual_feature_dims)
            compressed_dims = best_state.get('compressed_dims', 32)
            selected_indices = best_state.get('selected_feature_indices')

        else:
            # Simple checkpoint format (just state dict)
            logger.info("Loading simple checkpoint format")

            # If it's a full state dict with 'state_dict' key
            if 'state_dict' in loaded:
                state_dict = loaded['state_dict']
            else:
                # It is the state dict itself
                state_dict = loaded

            # Extract metadata from config or use defaults
            feature_dims = self.config['model'].get('feature_dims', 128)
            compressed_dims = self.config['model'].get('compressed_dims', 32)
            selected_indices = None

            # Try to get selected indices from the config
            if 'selected_feature_indices' in loaded:
                selected_indices = loaded['selected_feature_indices']

        # CRITICAL: Verify dimensions match
        if 'compressed_dims' in state_dict:
            # Check if compressed_dims is in the state dict
            if hasattr(model, 'compressed_dims'):
                saved_compressed_dims = compressed_dims
                if saved_compressed_dims != model.compressed_dims:
                    logger.warning(f"Compressed dimension mismatch: saved={saved_compressed_dims}, model={model.compressed_dims}")
                    # Update model to match saved dimensions
                    model.compressed_dims = saved_compressed_dims
                    self.actual_feature_dims = saved_compressed_dims
        ...
        if selected_indices is not None:
            ...
            logger.info(f"Restored feature selection: {len(model._selected_feature_indices)} features")
        else:
            logger.info("No feature selection indices found in checkpoint")

        # Restore cluster centers if present
        if 'cluster_centers' in state_dict and state_dict['cluster_centers'] is not None:
            cluster_centers = state_dict['cluster_centers']
            if isinstance(cluster_centers, torch.Tensor):
                if cluster_centers.shape[1] == model.compressed_dims:
                    model.cluster_centers = nn.Parameter(cluster_centers.to(self.device))
                    logger.info(f"Restored cluster centers: {cluster_centers.shape}")
                else:
                    logger.warning(f"Cluster centers dimension mismatch: {cluster_centers.shape[1]} vs {model.compressed_dims}")

        # Restore classifier if present
        if 'classifier_state' in state_dict and state_dict['classifier_state'] is not None:
            if hasattr(model, 'classifier') and model.classifier is not None:
                try:
                    model.classifier.load_state_dict(state_dict['classifier_state'])
                    logger.info("Restored classifier state")
                except Exception as e:
        ...
        logger.info(f"Model loaded: compressed_dims={model.compressed_dims}, feature_selection_frozen={model._is_feature_selection_frozen}")
        return model

    def predict_images(self, data_path: str, output_csv: str = None, batch_size: int = 128, generate_heatmaps: bool = True):
        """Predict using frozen feature selection with COMPLETE state restoration and deterministic behavior"""

        # =========================================================================
        # STEP 1: SET DETERMINISTIC BEHAVIOR FOR REPRODUCIBILITY
        # =========================================================================
        ...
        # =========================================================================
        # STEP 2: LOAD IMAGE FILES WITH LABELS
        # =========================================================================
        ...
        # Load training class mapping to preserve target labels
        ...
        # =========================================================================
        # STEP 4: INITIALIZE PREDICTION STORAGE - FIXED
        # =========================================================================
        all_predictions = {
            'filename': [],           # Will be populated during prediction
            'filepath': [],           # Will be populated during prediction (FIXED)
            'target': [],             # Will be populated during prediction (FIXED)
            'features_phase1': [],
            'class_predictions': [],
            'class_probabilities': [],
            'cluster_assignments': [],
            'cluster_confidence': [],
            'domain_features': []     # NEW: Store domain-specific features
        }

        # =========================================================================
        # STEP 5: ENSURE MODEL IS IN EVAL MODE AND FROZEN FEATURES ARE ACTIVE
        # =========================================================================
        self.model.eval()

        # CRITICAL: Verify feature selection is frozen
        if hasattr(self.model, '_is_feature_selection_frozen') and self.model._is_feature_selection_frozen:
            if hasattr(self.model, '_selected_feature_indices') and self.model._selected_feature_indices is not None:
                logger.info(f"✓ Using frozen feature selection with {len(self.model._selected_feature_indices)} features")
                logger.info(f"  Feature indices: {self.model._selected_feature_indices[:10].tolist()}{'...' if len(self.model._selected_feature_indices) > 10 else ''}")
            else:
                logger.warning("⚠️ Feature selection is frozen but no indices found!")
        else:
            logger.warning("⚠️ Feature selection is NOT frozen! Predictions may be inconsistent across runs.")
            logger.warning("  Please retrain the model to save feature selection information.")

        # Log model configuration
        logger.info(f"Model configuration:")
        logger.info(f"  - Training phase: {self.model.training_phase}")
        logger.info(f"  - KL divergence: {self.model.use_kl_divergence}")
        logger.info(f"  - Class encoding: {self.model.use_class_encoding}")
        logger.info(f"  - Feature dimensions: {self.actual_feature_dims}")

        # =========================================================================
        # STEP 6: REGISTER ATTENTION HOOKS FOR HEATMAPS
        # =========================================================================
        ...
        # =========================================================================
        # STEP 7: PREDICTION LOOP WITH DETERMINISTIC BEHAVIOR
        # =========================================================================
        with torch.no_grad():
            for batch_idx, batch_data in enumerate(tqdm(dataloader, desc="Predicting")):
                ...
                # Handle single-sample batches for consistent behavior
                ...
                if original_batch_size == 1:
                    ...
                    duplicated = True
                    logger.debug(f"Duplicated single-sample batch for consistent processing")

                batch_tensor = batch_tensor.to(self.device)

                # =================================================================
                # CRITICAL: Use deterministic prediction method if available
                # =================================================================
                if hasattr(self.model, 'predict_deterministic'):
                    output = self.model.predict_deterministic(batch_tensor)
                    logger.debug("Using deterministic prediction method")
                else:
                    # Fallback to standard forward pass
                    output = self.model(batch_tensor)
                    logger.debug("Using standard forward pass (no deterministic method)")

                # =================================================================
                # EXTRACT FEATURES WITH FROZEN SELECTION
                # =================================================================
                ...
                # Apply frozen feature selection
                if hasattr(self.model, '_is_feature_selection_frozen') and self.model._is_feature_selection_frozen:
                    if hasattr(self.model, 'get_frozen_features'):
                        ...
                        logger.warning("Frozen feature selection flag set but get_frozen_features missing")
                else:
                    features = embedding.detach().cpu().numpy()

                # Handle duplicated batch
                if duplicated:
                    features = features[:original_batch_size]
                    logger.debug(f"Removed duplicated sample, features shape: {features.shape}")

                # =================================================================
                # VALIDATE FEATURE DIMENSIONS
                # =================================================================
                expected_dims = self.actual_feature_dims

                if features.shape[1] != expected_dims:
                    logger.warning(f"Feature dimension mismatch: got {features.shape[1]}, expected {expected_dims}")

                    if features.shape[1] > expected_dims:
                        # Truncate to expected dimensions
                        features = features[:, :expected_dims]
                        logger.info(f"Truncated features to {expected_dims} dimensions")
                    else:
                        # Pad with zeros if needed
                        padded_features = np.zeros((features.shape[0], expected_dims), dtype=features.dtype)
                        padded_features[:, :features.shape[1]] = features
                        features = padded_features
                        logger.info(f"Padded features to {expected_dims} dimensions")

                # =================================================================
                # STORE PREDICTIONS WITH PROPER BATCH INDEXING - FIXED
                # =================================================================
                batch_size_actual = features.shape[0]
                start_idx = batch_idx * batch_size

                # Get filenames, full paths, and targets for this batch
                batch_filenames = []
                batch_full_paths = []
                batch_targets = []  # NEW: Store targets for this batch
                ...
                for i in range(batch_size_actual):
                    ...
                    if actual_idx < len(image_files):
                        ...
                        batch_targets.append(mapped_labels[actual_idx])  # NEW: Add target
                    else:
                        batch_filenames.append(f"batch_{batch_idx}_{i}")
                        batch_full_paths.append(f"batch_{batch_idx}_{i}")
                        batch_targets.append("unknown")  # NEW: Default target for edge cases

                # Store predictions
                all_predictions['filename'].extend(batch_filenames)
                all_predictions['filepath'].extend(batch_full_paths)
                all_predictions['target'].extend(batch_targets)  # NEW: Store targets
                all_predictions['features_phase1'].extend(features)


                # =================================================================
                # STORE ADDITIONAL OUTPUTS WITH LENGTH VALIDATION - FIXED
                # =================================================================
                # Class predictions
                if 'class_predictions' in output:
                    preds = output['class_predictions'].cpu().numpy()
                    if duplicated:
                        preds = preds[:original_batch_size]

                    # Validate length
                    if len(preds) != batch_size_actual:
                        logger.warning(f"Class predictions length mismatch: {len(preds)} vs {batch_size_actual}")
                        if len(preds) > batch_size_actual:
                            preds = preds[:batch_size_actual]
                        else:
                            # Pad with -1 for missing predictions
                            preds = np.concatenate([preds, np.full(batch_size_actual - len(preds), -1)])

                    all_predictions['class_predictions'].extend(preds)
                else:
                    # If no predictions available, pad with -1
                    all_predictions['class_predictions'].extend([-1] * batch_size_actual)

                # Class probabilities
                if 'class_probabilities' in output:
                    probs = output['class_probabilities'].cpu().numpy()
                    if duplicated:
                        probs = probs[:original_batch_size]

                    # Validate length
                    if len(probs) != batch_size_actual:
                        logger.warning(f"Class probabilities length mismatch: {len(probs)} vs {batch_size_actual}")
                        if len(probs) > batch_size_actual:
                            probs = probs[:batch_size_actual]
                        else:
                            # Pad with zeros for missing probabilities
                            num_classes = probs.shape[1] if probs.ndim > 1 else 2
                            padding = np.zeros((batch_size_actual - len(probs), num_classes))
                            probs = np.concatenate([probs, padding])

                    all_predictions['class_probabilities'].extend(probs)
                else:
                    # If no probabilities available, pad with zeros
                    num_classes = self.config['dataset'].get('num_classes', 2)
                    all_predictions['class_probabilities'].extend([np.zeros(num_classes)] * batch_size_actual)

                # Cluster assignments
                if 'cluster_assignments' in output:
                    clusters = output['cluster_assignments'].cpu().numpy()
                    if duplicated:
                        clusters = clusters[:original_batch_size]

                    # Validate length
                    if len(clusters) != batch_size_actual:
                        logger.warning(f"Cluster assignments length mismatch: {len(clusters)} vs {batch_size_actual}")
                        if len(clusters) > batch_size_actual:
                            clusters = clusters[:batch_size_actual]
                        else:
                            clusters = np.concatenate([clusters, np.full(batch_size_actual - len(clusters), -1)])

                    all_predictions['cluster_assignments'].extend(clusters)
                else:
                    all_predictions['cluster_assignments'].extend([-1] * batch_size_actual)

                # Cluster confidence
                if 'cluster_confidence' in output:
                    conf = output['cluster_confidence'].cpu().numpy()
                    if duplicated:
                        conf = conf[:original_batch_size]

                    # Validate length
                    if len(conf) != batch_size_actual:
                        logger.warning(f"Cluster confidence length mismatch: {len(conf)} vs {batch_size_actual}")
                        if len(conf) > batch_size_actual:
                            conf = conf[:batch_size_actual]
                        else:
                            conf = np.concatenate([conf, np.zeros(batch_size_actual - len(conf))])

                    all_predictions['cluster_confidence'].extend(conf)
                else:
                    all_predictions['cluster_confidence'].extend([0.0] * batch_size_actual)

                # =================================================================
                # EXTRACT DOMAIN-SPECIFIC FEATURES (NEW)
                # =================================================================
                if hasattr(self, '_extract_domain_features'):
                    for i in range(batch_size_actual):
                        actual_idx = start_idx + i
                        if actual_idx < len(image_files):
                            try:
                                domain_feat = self._extract_domain_features(image_files[actual_idx])
                                all_predictions['domain_features'].append(domain_feat)
                            except Exception as e:
                                logger.warning(f"Could not extract domain features: {e}")
                                all_predictions['domain_features'].append({})

                # =================================================================
                # GENERATE BATCH HEATMAPS
                # =================================================================
        ...
        # =========================================================================
        # STEP 8: GENERATE CLASSWISE HEATMAPS
        # =========================================================================
        ...
        # =========================================================================
        # STEP 9: SAVE PREDICTIONS WITH COMPLETE METADATA
        # =========================================================================
        if output_csv:
            self._save_predictions(all_predictions, output_csv)

            # Log feature statistics
            features_array = np.array(all_predictions['features_phase1'])
            logger.info(f"Features shape: {features_array.shape}")
            logger.info(f"Feature range: [{features_array.min():.4f}, {features_array.max():.4f}]")
            logger.info(f"Feature mean: {features_array.mean():.4f}, std: {features_array.std():.4f}")

            # Verify consistency with training files
            dataset_name = self.config['dataset']['name'].lower()
            ...
            # Save feature selection metadata separately for debugging
            if hasattr(self.model, '_selected_feature_indices') and self.model._selected_feature_indices is not None:
                metadata_path = output_csv.replace('.csv', '_feature_selection.json')
                feature_selection_info = {
                    'num_features': len(self.model._selected_feature_indices),
                    'indices': self.model._selected_feature_indices.tolist(),
        ...
        # =========================================================================
        # STEP 10: LOG FINAL STATISTICS
        # =========================================================================
        logger.info("=" * 60)
        logger.info("PREDICTION COMPLETED")
        logger.info(f"  - Images processed: {len(image_files)}")
        logger.info(f"  - Features dimension: {features_array.shape[1] if 'features_array' in locals() else 'N/A'}")
        logger.info(f"  - Known classes: {len(known_labels)}")
        logger.info(f"  - Unknown samples: {len(unknown_labels)}")

        # Log class distribution if available
        if all_predictions['class_predictions']:
            from collections import Counter
            pred_counts = Counter(all_predictions['class_predictions'])
            logger.info(f"  - Prediction distribution: {dict(pred_counts)}")

        # Log cluster distribution if available
        if all_predictions['cluster_assignments']:
            cluster_counts = Counter(all_predictions['cluster_assignments'])
    ...
    def _save_simple_csv(self, predictions: Dict[str, np.ndarray], output_csv: str):
        ...
        try:
            # First, determine the expected number of samples
            num_samples = None
            for key, values in predictions.items():
            ...
            if num_samples is None:
                logger.error("Could not determine number of samples for CSV")
                return
            ...
            for key, values in predictions.items():
                if key == 'features_phase1':
                    # Flatten feature arrays - ensure they're properly shaped
                    if isinstance(values, np.ndarray) and values.ndim == 2:
                        for i in range(values.shape[1]):
                            df_data[f'feature_{i}'] = values[:, i]
                    else:
                        logger.warning(f"Unexpected features shape for {key}: {values.shape if hasattr(values, 'shape') else 'unknown'}")

                elif key in ['class_probabilities', 'cluster_probabilities']:
                    # Handle probability arrays
                    if isinstance(values, np.ndarray) and values.ndim == 2:
                        for i in range(values.shape[1]):
                            df_data[f'{key}_{i}'] = values[:, i]
                    elif isinstance(values, np.ndarray) and values.ndim == 1:
                        df_data[key] = values
                    else:
                        logger.warning(f"Unexpected shape for {key}: {values.shape if hasattr(values, 'shape') else 'unknown'}")

                elif key in ['class_predictions', 'cluster_assignments', 'cluster_confidence']:
                    # Handle single-value predictions
                    if isinstance(values, np.ndarray):
                        # Ensure we have the right length
                        if len(values) != num_samples:
                            logger.warning(f"Length mismatch for {key}: {len(values)} vs {num_samples}")
                            # Truncate or pad if needed
                            if len(values) > num_samples:
                                df_data[key] = values[:num_samples]
                            else:
                                # Pad with zeros
                                padded = np.zeros(num_samples, dtype=values.dtype)
                                padded[:len(values)] = values
                                df_data[key] = padded
                        else:
                            df_data[key] = values

                elif key in ['filename', 'filepath', 'target']:
                    # Handle string lists
                    if isinstance(values, np.ndarray):
                        if len(values) != num_samples:
                            logger.warning(f"Length mismatch for {key}: {len(values)} vs {num_samples}")
                            if len(values) > num_samples:
                                ...
                                padded = np.array([''] * num_samples, dtype=object)
                                padded[:len(values)] = values
                                df_data[key] = padded
                        else:
                            df_data[key] = values
            ...
            # Create DataFrame and ensure all columns have the same length
            if df_data:
                # Verify all arrays have the same length
                for col_name, col_data in df_data.items():
                    if len(col_data) != num_samples:
                        logger.error(f"Column {col_name} has length {len(col_data)}, expected {num_samples}")
                        # Fix by truncating or padding
                        if len(col_data) > num_samples:
                            df_data[col_name] = col_data[:num_samples]
                        else:
                            # Pad with appropriate default values
                            if isinstance(col_data, np.ndarray):
                                if col_data.dtype.kind in ['S', 'U']:  # String type
                                    padded = np.array([''] * num_samples, dtype=col_data.dtype)
                                else:  # Numeric type
                                    padded = np.zeros(num_samples, dtype=col_data.dtype)
                                padded[:len(col_data)] = col_data
                                df_data[col_name] = padded

                df = pd.DataFrame(df_data)

                # Ensure target column is first if it exists
                if 'target' in df.columns:
                    cols = ['target'] + [col for col in df.columns if col != 'target']
                    df = df[cols]

                # Check if file exists before saving (already handled in _save_predictions, but double-check)
                if os.path.exists(output_csv):
                    logger.info(f"File {output_csv} already exists, it will be overwritten")

                df.to_csv(output_csv, index=False)
                logger.info(f"Simple predictions CSV saved to {output_csv}")
            ...
            logger.error(f"Predictions keys: {predictions.keys()}")
            for key, values in predictions.items():
                if isinstance(values, np.ndarray):
                    logger.error(f"  {key}: shape={values.shape}, dtype={values.dtype}")

    def _extract_domain_features(self, image_path: str) -> Dict[str, float]:
        """Extract domain-specific features from an image - NEW HELPER METHOD"""
        domain_features = {}

        try:
            # Get domain from config
            domain = self.config['dataset'].get('domain', 'general')

            if domain == 'astronomy':
                # Check if it's a FITS file
                if image_path.lower().endswith(('.fits', '.fit', '.fits.gz')):
                    # Use astronomy processor if available
                    if hasattr(self, '_astronomy_processor') and self._astronomy_processor:
                        img_array = self._astronomy_processor.read_fits(image_path)
                        if img_array is not None:
                            domain_features = self._astronomy_processor.extract_features(img_array)
                    else:
                        logger.debug("Astronomy processor not available for feature extraction")
                else:
                    # Standard image processing for astronomy
                    try:
                        ...
                        logger.debug(f"Could not extract astronomy features: {e}")
                ...
                try:
                    ...
                    domain_features = {
                        ...
                        'sharpness': float(np.mean(np.abs(np.gradient(img_array)[0])))
                    }
                except Exception as e:
                    logger.debug(f"Could not extract medical features: {e}")

            elif domain == 'agriculture':
                # Extract agriculture features
                try:
                    img = PILImage.open(image_path)
                    img_array = np.array(img) / 255.0
                    # Simple NDVI approximation from RGB
                    if img_array.shape[-1] >= 3:
                        nir = img_array[:, :, 2]  # Approximate NIR as red channel
                        red = img_array[:, :, 0]
                        ndvi = (nir - red) / (nir + red + 1e-8)
                        domain_features = {
                            'ndvi_mean': float(np.mean(ndvi)),
                            'greenness': float(np.mean(img_array[:, :, 1]))
                        }
                except Exception as e:
                    logger.debug(f"Could not extract agriculture features: {e}")

        except Exception as e:
            logger.warning(f"Error extracting domain features from {image_path}: {e}")
...
class BaseAutoencoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, labels: Optional[torch.Tensor] = None) -> Dict[str, torch.Tensor]:
        """Forward pass with consistent latent space"""

        # Step 1: Encode to full embedding space
        embedding = self.encode(x)
        if isinstance(embedding, tuple):
            embedding = embedding[0]

        # Step 2: Apply feature selection if frozen
        ...
        # Step 3: Compress to latent space (this is the KEY latent representation)
        ...
        # Step 5: Add classification and clustering (both operate on compressed_embedding)
...
#--------------------------Distance Correlations ----------
    ...
    def verify_feature_consistency(self, train_csv: str, test_csv: str, pred_csv: str) -> bool:
        """Verify feature consistency across training, test, and prediction - NEW METHOD"""
        try:
            ...
            if len(train_features) != len(test_features) or len(train_features) != len(pred_features):
                logger.error(f"Feature dimension mismatch: train={len(train_features)}, "
                            f"test={len(test_features)}, pred={len(pred_features)}")
                return False

            # Check feature indices match
            if set(train_features) != set(test_features) or set(train_features) != set(pred_features):
                logger.error("Feature columns don't match across files")
                return False

            # Check if feature selection was applied correctly
            expected_features = len(self._selected_feature_indices) if self._selected_feature_indices is not None else self.compressed_dims

            if len(train_features) != expected_features:
                logger.warning(f"Expected {expected_features} features, got {len(train_features)}")

            logger.info(f"✓ Feature consistency verified: {len(train_features)} features")
            return True
...
#--------------------------
    def _initialize_clustering(self, config: Dict):
        """Initialize clustering parameters with existence check - FIXED to use compressed dims"""
        enhancements = config['model']['autoencoder_config']['enhancements']
        self.use_kl_divergence = enhancements.get('use_kl_divergence', True)
        self.use_class_encoding = enhancements.get('use_class_encoding', True)
        ...
        if self.use_kl_divergence:
            ...
            if not hasattr(self, 'cluster_centers'):
                ...
                # CRITICAL FIX: Use compressed_dims for cluster centers, not feature_dims
                self.register_buffer('cluster_centers',
                                   torch.randn(num_clusters, self.compressed_dims))
                logger.info(f"Initialized cluster centers: {num_clusters} clusters x {self.compressed_dims}D")

        # ALWAYS initialize temperature as a tensor buffer
        temp_value = enhancements.get('clustering_temperature', 1.0)
        if not hasattr(self, 'clustering_temperature'):
            ...
            logger.info(f"Initialized clustering temperature: {temp_value}")
        else:
    ...
    def load_state_dict(self, state_dict, strict: bool = True):
        ...
        if 'classifier_state' in state_dict and hasattr(self, 'classifier'):
            try:
                # FIX: Try to load with strict=False to handle architecture mismatches
                self.classifier.load_state_dict(state_dict['classifier_state'], strict=False)
                logger.info("Classifier state loaded successfully")
            except Exception as e:
                logger.warning(f"Could not load classifier state: {e}")
                # If loading fails, we may need to reinitialize the classifier
                if hasattr(self, '_classifier_initialized'):
                    # Mark as uninitialized so it will be recreated when needed
                    self._classifier_initialized = None
                logger.warning("Classifier will be reinitialized during forward pass if needed")

        if strict:
            # Only report missing keys that are truly missing (ignore classifier keys if they're optional)
            if missing_keys:
                # Filter out classifier keys if classifier is optional
                if hasattr(self, 'use_class_encoding') and not self.use_class_encoding:
...
#--------------------------
    ...
    def _initialize_latent_organization(self):
        ...
        enhancements = self.config['model'].get('autoencoder_config', {}).get('enhancements', {})
        self.use_kl_divergence = enhancements.get('use_kl_divergence', True)
        self.use_class_encoding = enhancements.get('use_class_encoding', True)

        # Initialize classifier with compressed_dims (latent space)
        if self.use_class_encoding and not hasattr(self, 'classifier'):
            num_classes = self.config['dataset'].get('num_classes', 10)
            self.classifier = nn.Sequential(
                nn.Linear(self.compressed_dims, max(self.compressed_dims // 2, 16)),
                nn.ReLU(),
                nn.Dropout(0.3),
                nn.Linear(max(self.compressed_dims // 2, 16), num_classes)
            ).to(self.device)
            self._classifier_initialized = self.compressed_dims
            logger.info(f"Initialized classifier: {self.compressed_dims}D → {num_classes} classes")

        # For backward compatibility: if classifier exists but dimensions changed
        if (self.use_class_encoding and hasattr(self, 'classifier') and
            hasattr(self, '_classifier_initialized') and
            self._classifier_initialized != self.compressed_dims):
            logger.warning(f"Classifier dimension mismatch: expected {self._classifier_initialized}, got {self.compressed_dims}")
            # Reinitialize classifier with correct dimensions
            num_classes = self.config['dataset'].get('num_classes', 10)
            self.classifier = nn.Sequential(
                nn.Linear(self.compressed_dims, max(self.compressed_dims // 2, 16)),
                nn.ReLU(),
            ...
            self._classifier_initialized = self.compressed_dims
            logger.info(f"Reinitialized classifier: {self.compressed_dims}D → {num_classes} classes")
    ...
    def organize_latent_space(self, compressed_embedding: torch.Tensor,
                              labels: Optional[torch.Tensor] = None) -> Dict[str, torch.Tensor]:
        """
        Organize latent space with KL divergence and classification.

        IMPORTANT: This operates on COMPRESSED_EMBEDDING (latent space)
        to ensure training and prediction use the same representation.

        Args:
            compressed_embedding: Tensor of shape [batch_size, compressed_dims]
            labels: Optional ground truth labels for training

        Returns:
            Dictionary with clustering and classification outputs
        """
        output = {'compressed_embedding': compressed_embedding}

        # =====================================================================
        # KL Divergence Clustering - on latent space
        # =====================================================================
        if self.use_kl_divergence and hasattr(self, 'cluster_centers'):
            cluster_centers = self.cluster_centers.to(compressed_embedding.device)

            # CRITICAL: Verify dimensions match
            if cluster_centers.shape[1] != compressed_embedding.shape[1]:
                error_msg = (f"Dimension mismatch in organize_latent_space: "
                            f"cluster_centers {cluster_centers.shape[1]}D, "
                            f"compressed_embedding {compressed_embedding.shape[1]}D. "
                            f"Expected both to be {self.compressed_dims}D.")
                logger.error(error_msg)
                raise ValueError(error_msg)

            # Calculate distances to cluster centers
            distances = torch.cdist(compressed_embedding, cluster_centers)

            # Get temperature (ensure it's a scalar)
            temperature = self.clustering_temperature.item() if hasattr(self.clustering_temperature, 'item') else float(self.clustering_temperature)

            # Student's t-distribution for soft assignments (standard in DEC/IDEC)
            q_dist = torch.pow(1.0 + distances / temperature, -(temperature + 1) / 2)
            q_dist = q_dist / (q_dist.sum(dim=1, keepdim=True) + 1e-8)

            # Create target distribution for KL divergence
            if labels is not None and self.training:
                # During training, use ground truth to guide clustering
                ...
                # During prediction, use sharpened distribution (self-supervised)
    ...
    def predict_deterministic(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        """
        Deterministic prediction that EXACTLY mirrors training forward pass.
        """
        self.eval()
        ...
        # EXACT SAME FLOW AS forward() method
        embedding = self.encode(x)
        if isinstance(embedding, tuple):
            embedding = embedding[0]
        ...
        output = {
            ...
            'compressed_embedding': self._fix_tensor_dim(compressed_embedding, original_batch_size, duplicated),
            'reconstruction': self._fix_tensor_dim(reconstruction, original_batch_size, duplicated)
        }

        # Add classification and clustering (SAME as organize_latent_space during training)
        if self.training_phase == 2:
            # Classification
            if self.use_class_encoding and hasattr(self, 'classifier') and self.classifier is not None:
                logits = self.classifier(compressed_embedding)
                output.update({
            ...
            if self.use_kl_divergence and hasattr(self, 'cluster_centers') and self.cluster_centers is not None:
                # CRITICAL: Verify dimensions match
                if self.cluster_centers.shape[1] != compressed_embedding.shape[1]:
                    error_msg = (f"CRITICAL: Dimension mismatch in prediction! "
                                f"Cluster centers: {self.cluster_centers.shape[1]}D, "
                                f"Compressed embedding: {compressed_embedding.shape[1]}D. "
                                f"Expected both to be {self.compressed_dims}D.")
                    logger.error(error_msg)
                    raise RuntimeError(error_msg)

                distances = torch.cdist(compressed_embedding, self.cluster_centers)
                temperature = self.clustering_temperature.item() if hasattr(self.clustering_temperature, 'item') else float(self.clustering_temperature)

                # Student's t-distribution (same as training)
                q = torch.pow(1.0 + distances / temperature, -(temperature + 1) / 2)
    ...
    def _fix_tensor_dim(self, tensor: torch.Tensor, target_batch_size: int, duplicated: bool) -> torch.Tensor:
        """Fix tensor dimensions after duplication - HELPER METHOD"""
...
class MedicalStructureLoss(nn.Module):
    """Loss function specialized for medical imaging features - NEW"""
    def __init__(self):
        super().__init__()
        self.sobel_x = torch.tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]],
                                     dtype=torch.float32).view(1, 1, 3, 3)
...
class AgriculturalPatternLoss(nn.Module):
    """Loss function optimized for agricultural pest and disease detection - NEW"""
    def __init__(self):
        super().__init__()
...
class UnifiedCheckpoint:
    ...
    def load_checkpoint(self):
        """Load existing checkpoint or initialize new one - FIXED for both unified and simple checkpoints"""
        if os.path.exists(self.checkpoint_path):
            try:
                # Try to load with weights_only=False first (for compatibility with existing checkpoints)
                loaded = torch.load(self.checkpoint_path, map_location='cpu', weights_only=False)

                # Check if this is a unified checkpoint (has model_states key) or a simple checkpoint
                if isinstance(loaded, dict) and 'model_states' in loaded:
                    self.current_state = loaded
                    logger.info(f"Loaded unified checkpoint from {self.checkpoint_path}")
                else:
                    # This is a simple checkpoint (just state dict)
                    # Convert it to unified format
                    logger.info(f"Loading simple checkpoint and converting to unified format")

                    # Determine phase from the checkpoint
                    phase = 2  # Default to phase 2
                    if 'phase' in loaded:
                        phase = loaded.get('phase', 2)
                    ...
                    # Create a model state entry
                    state_key = self.get_state_key(phase, None)  # Pass None for model

                    # Extract necessary info
                    epoch = loaded.get('epoch', 0)
                    loss = loaded.get('loss', float('inf'))
                    ...
                    self.current_state['model_states'][state_key] = {
                        'current': {
                            ...
                            'phase': phase,
                            'loss': loss,
                            'timestamp': loaded.get('timestamp', datetime.now().isoformat()),
                            'config': loaded.get('config', {})
                        },
                        'best': loaded if loaded.get('best', False) else None,
                        'history': []
                    }

                    logger.info(f"Converted simple checkpoint to unified format")

            except Exception as e:
                logger.error(f"Failed to load checkpoint: {e}")
                logger.info("Initializing new checkpoint due to load failure")
                self.current_state = {
                    'model_states': {},
                    'metadata': {
                        'created_at': datetime.now().isoformat(),
    ...
    def save_model_state(self, model: nn.Module, optimizer: torch.optim.Optimizer,
        ...
        """Save model state with COMPLETE state - FIXED"""
        state_key = self.get_state_key(phase, model)

        # CRITICAL: Save all dimensions
        state_dict = {
            ...
            # CRITICAL: Save all dimensions for consistent restoration
            'feature_dims': int(getattr(model, 'feature_dims', 128)),
            'compressed_dims': int(getattr(model, 'compressed_dims', 32)),

            # Feature selection state
            'selected_feature_indices': getattr(model, '_selected_feature_indices', None),
            'is_feature_selection_frozen': getattr(model, '_is_feature_selection_frozen', False),

            # Classifier and clustering states
            'classifier_state': model.classifier.state_dict() if hasattr(model, 'classifier') and model.classifier is not None else None,
            'cluster_centers': getattr(model, 'cluster_centers', None),
            'clustering_temperature': getattr(model, 'clustering_temperature', None),

            # Configuration
            'config': {
                'feature_dims': int(getattr(model, 'feature_dims', 128)),
                'compressed_dims': int(getattr(model, 'compressed_dims', 32)),
                'use_kl_divergence': bool(model.use_kl_divergence),
                'use_class_encoding': bool(model.use_class_encoding),
                'num_classes': int(self.config['dataset'].get('num_classes', 10))
            }
        }

        # Save with weights_only=False to preserve all types
        torch.save(state_dict, self.checkpoint_path, _use_new_zipfile_serialization=False)
        logger.info(f"Saved model state with compressed_dims={model.compressed_dims}")

    def load_model_state(self, model: nn.Module, optimizer: torch.optim.Optimizer,
                        phase: int, load_best: bool = False) -> Optional[Dict]:
        """Load model state including feature selection configuration - FIXED"""
        state_key = self.get_state_key(phase, model)
        ...
        if 'config' in state_dict and 'feature_selection' in state_dict['config']:
            ...
            model._is_feature_selection_frozen = fs_state.get('is_frozen', True)
        ...
        if 'cluster_centers' in state_dict and state_dict['cluster_centers'] is not None:
            if hasattr(model, 'cluster_centers'):
                try:
                    if isinstance(state_dict['cluster_centers'], torch.Tensor):
                        model.cluster_centers.data = state_dict['cluster_centers'].to(model.device)
                    else:
                        model.cluster_centers.data = torch.tensor(state_dict['cluster_centers'], device=model.device)
                    logger.info(f"Restored cluster centers: {model.cluster_centers.shape}")
                except Exception as e:
                    logger.warning(f"Could not restore cluster centers: {e}")

        # Restore clustering temperature
        if 'clustering_temperature' in state_dict and state_dict['clustering_temperature'] is not None:
            if hasattr(model, 'clustering_temperature'):
                try:
                    if isinstance(state_dict['clustering_temperature'], torch.Tensor):
                        model.clustering_temperature.data = state_dict['clustering_temperature'].to(model.device)
                    else:
                        model.clustering_temperature.data = torch.tensor(state_dict['clustering_temperature'], device=model.device)
                    logger.info(f"Restored clustering temperature: {model.clustering_temperature.item():.3f}")
                except Exception as e:
                    logger.warning(f"Could not restore clustering temperature: {e}")
    ...
    def get_state_key(self, phase: int, model: nn.Module = None) -> str:
        """Generate unique key including model type"""
        components = [f"phase{phase}", f"model_{self.model_type}"]

        if model is not None and phase == 2:
            if hasattr(model, 'use_kl_divergence') and model.use_kl_divergence:
                components.append("kld")
            if hasattr(model, 'use_class_encoding') and model.use_class_encoding:
...
class ModelFactory:
    ...
    def create_model(config: Dict) -> nn.Module:
        """Create model with domain-specific enhancements - MODIFIED"""
        input_shape = (
        ...
        # Create domain-specific model if available
        if domain == 'astronomy' or image_type == 'astronomical':
            try:
                # Try to import astronomical model
                from astronomical_autoencoder import AstronomicalStructurePreservingAutoencoder
                model = AstronomicalStructurePreservingAutoencoder(input_shape, feature_dims, config)
                logger.info("Created Astronomical Structure Preserving Autoencoder")
                return model
            except ImportError:
                logger.warning("Astronomical model not available, using base autoencoder")

        elif domain == 'medical' or image_type == 'medical':
            try:
                from medical_autoencoder import MedicalStructurePreservingAutoencoder
                model = MedicalStructurePreservingAutoencoder(input_shape, feature_dims, config)
                logger.info("Created Medical Structure Preserving Autoencoder")
                return model
            except ImportError:
                logger.warning("Medical model not available, using base autoencoder")

        elif domain == 'agriculture' or image_type == 'agricultural':
            try:
                from agricultural_autoencoder import AgriculturalPatternAutoencoder
                model = AgriculturalPatternAutoencoder(input_shape, feature_dims, config)
                logger.info("Created Agricultural Pattern Autoencoder")
                return model
            except ImportError:
                logger.warning("Agricultural model not available, using base autoencoder")

        # Default to base autoencoder
        model = BaseAutoencoder(input_shape, feature_dims, config)
        logger.info(f"Created base autoencoder with {feature_dims}D → {config['model']['compressed_dims']}D compression")
        return model
    ...
    try:
        for epoch in range(start_epoch, epochs):
            ...
            for batch_idx, (data, labels) in enumerate(pbar):
                ...
                # FIX: Handle single-sample batches by skipping them or accumulating
                if data.size(0) == 1:
                    logger.warning(f"Single-sample batch detected at index {batch_idx}, skipping BatchNorm-sensitive operations")
                    # For single-sample batches, we need to handle them specially
                    # Option 1: Skip them (simple)
                    continue

                    # Option 2: If we must process them, we can use eval mode temporarily
                    # But for simplicity, we'll skip single-sample batches during training
                ...
                try:
                    if phase == 1:
                        ...
                        if model.use_class_encoding and 'class_logits' in output:
                            ...
                            if use_classwise and len(torch.unique(labels)) > 1:
                                class_losses = []
                ...
                if phase == 2 and model.use_class_encoding:
                    avg_acc = running_acc / (batch_idx + 1) if valid_batches > 0 else 0
...
class ReconstructionManager:
    ...
    def _save_predictions(self, predictions: Dict[str, np.ndarray], output_csv: str):
        """Save predictions with comprehensive metadata and binary format support"""
        # Determine output directory from the CSV path
        output_dir = os.path.dirname(output_csv)
        if output_dir:
            os.makedirs(output_dir, exist_ok=True)

        # Track the expected length from the first array
        expected_length = None

        # First pass: determine expected length and clean up predictions
        cleaned_predictions = {}
        ...
        for key, values in predictions.items():
            ...
            if isinstance(values, np.ndarray) and values.size == 0:
                logger.warning(f"Empty array for key {key}, skipping")
                continue

            # Handle numpy arrays
            if isinstance(values, np.ndarray):
                # Skip if array has no elements
                if values.size == 0:
                    continue

                # Get expected length from first non-empty array
                ...
                cleaned_predictions[key] = values
            else:
                    ...
                    if arr.size > 0:
                        if expected_length is None:
                            if arr.ndim == 1:
                                ...
                                expected_length = arr.shape[0]
                        cleaned_predictions[key] = arr
                except:
                    logger.warning(f"Could not convert {key} to numpy array, skipping")
                    continue
        ...
        # Create DataFrame with proper length validation
        df_data = {}

        for key, values in cleaned_predictions.items():
            if key == 'features_phase1':
                ...
                if isinstance(values, np.ndarray) and values.ndim == 2:
                    for i in range(values.shape[1]):
                        ...
                        if len(col_data) != expected_length:
                            if len(col_data) > expected_length:
                                col_data = col_data[:expected_length]
                            else:
                                padded = np.zeros(expected_length, dtype=col_data.dtype)
                                padded[:len(col_data)] = col_data
                                col_data = padded
                ...
                if isinstance(values, np.ndarray) and values.ndim == 2:
                    ...
                    if len(col_data) != expected_length:
                        if len(col_data) > expected_length:
                            ...
                            padded[:len(col_data)] = col_data
                            col_data = padded
                    df_data[key] = col_data

            elif key in ['class_predictions', 'cluster_assignments', 'cluster_confidence']:
                # Handle single-value predictions
                if isinstance(values, np.ndarray):
                    col_data = values.flatten() if values.ndim > 1 else values
                    if len(col_data) != expected_length:
                        if len(col_data) > expected_length:
                            col_data = col_data[:expected_length]
                        else:
                            if col_data.dtype.kind in ['S', 'U']:  # String type
                                padded = np.array([''] * expected_length, dtype=col_data.dtype)
                            else:  # Numeric type
                                padded = np.zeros(expected_length, dtype=col_data.dtype)
                            padded[:len(col_data)] = col_data
                            col_data = padded
                ...
                if isinstance(values, np.ndarray):
                    ...
                    if len(col_data) != expected_length:
                        if len(col_data) > expected_length:
                            ...
                            padded = np.array([''] * expected_length, dtype=object)
                            padded[:len(col_data)] = col_data
                            col_data = padded
                    df_data[key] = col_data
                ...
                if len(col_data) == expected_length:
                    ...
                    logger.warning(f"Skipping {key} due to length mismatch: {len(col_data)} vs {expected_length}")

        # Create and save DataFrame
        if df_data:
            # Verify all columns have the correct length
            for col_name, col_data in df_data.items():
                if len(col_data) != expected_length:
                    logger.error(f"Column {col_name} still has length {len(col_data)}, expected {expected_length}")
                    # Final fix: truncate or pad
                    if len(col_data) > expected_length:
                        df_data[col_name] = col_data[:expected_length]
                    else:
                        if isinstance(col_data, np.ndarray):
...
#------------------------
    ...
    def configure_domain_enhancements(self, config: Dict, domain: str = 'general') -> Dict:
        """Configure domain-specific enhancements - NEW METHOD"""
        ...
        domain_configs = {
            'astronomy': {
                ...
                'dataset': {
                    'image_type': 'astronomical',
                    'domain': 'astronomy',
                    'fits_support': True,
                    'fits_normalization': 'zscale'
                }
            },
            'medical': {
                'enhancement_modules': {
        ...
        if domain in domain_configs:
            ...
            for section, values in domain_configs[domain].items():
            ...
            logger.info(f"Applied {domain} domain enhancements to configuration")
        else:
            logger.info(f"No specific enhancements for domain: {domain}")
...
def download_and_process_dataset(dataset_name):
    ...
    dataset_name_lower = dataset_name.lower()  # Use lowercase for config
    ...
    try:
        ...
        # CRITICAL FIX: Return a properly structured result
        return {
            'dataset_name': dataset_name_lower,  # Use lowercase for internal use
            'dataset_name_upper': dataset_name_upper,
            'data_dir': f"Data/{dataset_name_upper}",
            'train_dir': train_dir,
            'test_dir': test_dir,
            'input_path': f"Data/{dataset_name_upper}",  # The path to use for training
            'data_type': 'custom',  # After processing, treat as custom dataset
            'mode': 'train',
            'config': config,
            'is_local': False,
...
def interactive_dataset_selection():
    """Enhanced interactive dataset selection with local folder browser - FIXED"""
    ...
    while True:
        ...
        if choice == '1':
            ...
            if selected_dataset:
                ...
                if result:
                    # CRITICAL FIX: Set the arguments correctly
                    result['data_name'] = selected_dataset.lower()  # Use lowercase for data_name
                    result['input_path'] = f"Data/{selected_dataset.upper()}"  # Use uppercase for directory
                    result['data_type'] = 'custom'  # After processing, treat as custom dataset
                    result['mode'] = 'train'  # Default to train mode
                    result['dataset_name'] = selected_dataset.lower()  # For config

                    print(f"\n✓ Dataset ready: {selected_dataset}")
                    print(f"  Data path: {result['input_path']}")
                    print(f"  Dataset name: {result['data_name']}")
                    print("\nYou can now train using:")
                    print(f"  python cdbnn.py --mode train --data_name {result['data_name']} --data_path {result['input_path']} --data_type custom")

                    # CRITICAL: Return the result with all required fields
                    return result
                else:
            ...
            if selected_path:
                ...
                result = {
                    'dataset_name': dataset_name.lower(),  # Use lowercase for consistency
                    'dataset_name_upper': dataset_name.upper(),
                ...
                if os.path.exists(train_dir) and os.path.isdir(train_dir):
                    ...
                    result['test_dir'] = test_dir if os.path.exists(test_dir) else None
                    print("✓ Found train/test directory structure")
                else:
                    result['train_dir'] = selected_path
                    result['test_dir'] = None
...
def main():
    ...
    if (not hasattr(args, 'data_name') or not args.data_name) and \
       ...
       (hasattr(args, 'interactive') and args.interactive):
        ...
        print("\n" + "="*70)
        print("CDBNN - Convolutional Deep Bayesian Neural Network")
        print("="*70)
        ...
        # =====================================================================
        # CRITICAL FIX: Set the arguments based on selected dataset
        # =====================================================================

        # Set data_name (always use lowercase for internal consistency)
        args.data_name = dataset_info.get('dataset_name', 'dataset').lower()
        logger.info(f"Set data_name to: {args.data_name}")

        # Set input_path (the actual path to the data)
        args.input_path = dataset_info.get('input_path', dataset_info.get('data_dir', 'data'))
        logger.info(f"Set input_path to: {args.input_path}")

        # Set data_type (after processing torchvision, it becomes custom)
        args.data_type = dataset_info.get('data_type', 'custom')
        logger.info(f"Set data_type to: {args.data_type}")

        # Set mode (default to train)
        args.mode = dataset_info.get('mode', 'train')
        logger.info(f"Set mode to: {args.mode}")

        # Set additional fields if available
        if 'train_dir' in dataset_info:
            args.train_dir = dataset_info['train_dir']
            logger.info(f"Set train_dir to: {args.train_dir}")

        if 'test_dir' in dataset_info:
            args.test_dir = dataset_info['test_dir']
            logger.info(f"Set test_dir to: {args.test_dir}")

        # Set domain if specified
        if 'domain' in dataset_info:
            args.domain = dataset_info['domain']
            logger.info(f"Set domain to: {args.domain}")
    ...
    if not hasattr(args, 'data_name') or not args.data_name:
        logger.error("data_name is required. Please specify with --data_name or use interactive mode.")
        return 1

    if not hasattr(args, 'input_path') or not args.input_path:
        logger.error("input_path is required. Please specify with --input_path or use interactive mode.")
        return 1

    if not hasattr(args, 'mode') or not args.mode:
        logger.error("mode is required. Please specify with --mode (train/predict/reconstruct/heatmaps)")
        return 1
    ...
    if hasattr(args, 'data_name') and args.data_name:
        ...
        if (hasattr(args, 'data_type') and args.data_type == 'torchvision' and
            ...
            print("\nFalling back to interactive dataset selection...")
            dataset_info = interactive_dataset_selection()
            if dataset_info:
                args.data_name = dataset_info.get('dataset_name', 'dataset').lower()
                args.input_path = dataset_info.get('input_path', dataset_info.get('data_dir', 'data'))
    ...
    if (hasattr(args, 'data_type') and args.data_type == 'torchvision' and
        ...
        (not hasattr(args, 'input_path') or not args.input_path or args.input_path == args.data_name) and
    ...
    # =====================================================================
    dataset_name_lower = args.data_name.lower()
    output_data_dir = os.path.join('data', dataset_name_lower)
    os.makedirs(output_data_dir, exist_ok=True)

    # Set default output paths if not specified
    if not hasattr(args, 'output_csv') or not args.output_csv:
        args.output_csv = os.path.join(output_data_dir, f"{dataset_name_lower}.csv")

    if not hasattr(args, 'output_dir') or not args.output_dir:
        args.output_dir = output_data_dir
    ...
    if args.mode == 'predict':
        ...
        if not config_path:
            ...
            config_path = os.path.join(output_data_dir, f"{dataset_name_lower}.json")
        ...
        try:
            ...
            logger.error(f"Failed to initialize prediction manager: {e}")
            return 1
        ...
        if generate_heatmaps:
            if num_samples is None:
                logger.info("Heatmap generation enabled - creating attention visualizations for ALL samples")
            else:
                logger.info(f"Heatmap generation enabled - creating attention visualizations for {num_samples} samples per class")
        else:
        ...
        if generate_heatmaps:
            heatmap_dir = os.path.join(output_data_dir, 'attention_heatmaps')
            logger.info(f"Attention heatmaps saved to: {heatmap_dir}")

        logger.info(f"Prediction completed successfully! Output: {args.output_csv}")
        return 0
        ...
        try:
            ...
            logger.error(f"Failed to initialize reconstruction manager: {e}")
            return 1

        output_dir = getattr(args, 'output_dir', os.path.join(output_data_dir, 'reconstructions'))
        ...
        if num_samples is None:
            logger.info("Generating heatmaps for ALL samples")
        else:
        ...
        logger.info("Valid modes: train, predict, reconstruct, heatmaps")
        return 1
