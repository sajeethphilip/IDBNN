...
class UnifiedTrainingManager:
    def train_and_save(self, config, train_loader, val_loader=None):
        """Complete training without any feature extraction"""
        # 1. Train model
        ...
        checkpoint_path = os.path.join(config['training']['checkpoint_dir'],
                                   f"{config['dataset']['name']}_final.pth")
        torch.save({
            'model_state': model.state_dict(),
            'config': config
        }, checkpoint_path)

        # 3. Return path for prediction phase
        return checkpoint_path

class CertifiedPredictionManager:
    def __init__(self, model_path):
        """Load model in pure prediction mode"""
        checkpoint = torch.load(model_path, map_location='cpu')
        self.config = checkpoint['config']
        self.model = ModelFactory.create_model(self.config)
        self.model.load_state_dict(checkpoint['model_state'])

        # Force prediction-mode configuration
        self.model.set_training_phase(2)
        self.model.eval()

        # Initialize standard paths
        dataset_name = self.config['dataset']['name']
        self.default_csv = os.path.join('data', dataset_name, f"{dataset_name}.csv")
        os.makedirs(os.path.dirname(self.default_csv), exist_ok=True)

    def generate_standard_features(self, data_path=None, output_csv=None):
        """
        Generate features using certified prediction path
        Args:
            data_path: If None, uses training data from config
            output_csv: If None, uses <dataset_name>.csv in standard location
        """
        output_csv = output_csv or self.default_csv

        # Load dataset if not provided
        if data_path is None:
            data_path = os.path.join('data', self.config['dataset']['name'], 'train')

        # Standardized prediction process
        transform = self._get_transforms()
        image_files, _, _ = self._get_image_files_with_labels(data_path)

        with open(output_csv, 'w') as f:
            writer = csv.writer(f)
            writer.writerow(STANDARD_COLUMNS)  # Defined globally

            for batch in self._batch_generator(image_files, transform):
                with torch.no_grad():
                    output = self.model(batch)

                # Standardized output processing
                features = self._process_output(output)
                writer.writerows(self._format_rows(features))

        # Save companion config file
        self._save_config_metadata(output_csv)

        return output_csv

    def _process_output(self, output):
        """Standardized output processing matching prediction requirements"""
        if isinstance(output, dict):
            return output
        else:
            # Convert tuple output to standardized dict format
            embedding, _ = output
            return {
                'embedding': embedding,
                'cluster_assignments': torch.full((len(embedding),), -1),
                'cluster_probabilities': torch.zeros((len(embedding), 1))
            }

    def _save_config_metadata(self, csv_path):
        """Save config metadata alongside CSV"""
        meta = {
            'creation_date': datetime.now().isoformat(),
            'model_hash': hashlib.md5(str(self.model.state_dict()).encode()).hexdigest(),
            'columns': STANDARD_COLUMNS,
            'feature_dims': self.config['model']['feature_dims'],
            'input_spec': self.config['dataset']
        }
...
class UnifiedCLI:
    """Handles all command-line and interactive operations"""
    ...
    def _create_parser(self):
        parser = argparse.ArgumentParser(
            description="Unified Feature Extraction Pipeline",
            formatter_class=argparse.ArgumentDefaultsHelpFormatter)

        # Core modes
        group = parser.add_mutually_exclusive_group(required=True)
        group.add_argument('--train-then-predict', action='store_true',
                         help='Full training followed by certified prediction')
        group.add_argument('--train-only', action='store_true',
                         help='Training only (no feature extraction)')
        group.add_argument('--predict-only', metavar='MODEL_PATH',
                         help='Certified prediction using existing model')

        # Shared parameters
        parser.add_argument('--dataset', required=True,
                          help='Dataset name (must match config)')
        parser.add_argument('--data-path', default='./data',
                          help='Root directory for datasets')
        parser.add_argument('--output-csv',
                          help='Custom output CSV path (default: <dataset>.csv)')
        parser.add_argument('--interactive', action='store_true',
                          help='Enter interactive mode after operations')

        return parser

    def execute(self):
        args = self.parser.parse_args()
        self._load_config(args.dataset)

        if args.train_then_predict or args.train_only:
            self._run_training(args)

        if args.train_then_predict or args.predict_only:
            self._run_prediction(args)
    ...
    def _run_training(self, args):
        """Training-only path"""
        print(f"🚀 Starting certified training for {args.dataset}")
        trainer = UnifiedTrainingManager()
        model_path = trainer.train_and_save(
            config=self.config,
            train_loader=create_dataloader(Path(args.data_path)/args.dataset/'train'),
            val_loader=create_dataloader(Path(args.data_path)/args.dataset/'val')
        )
        print(f"✅ Training complete. Model saved to {model_path}")

        if args.train_only:
            print("🔍 Use --train-then-predict or --predict-only to generate features")

    def _run_prediction(self, args):
        """Certified prediction path"""
        model_path = (args.predict_only or
                     Path(self.config['training']['checkpoint_dir'])/f"{args.dataset}_final.pth")

        print(f"🔮 Starting certified prediction using {model_path}")
        predictor = CertifiedPredictionManager(model_path)

        output_csv = args.output_csv or f"{args.dataset}.csv"
        predictor.generate_standard_features(
            data_path=Path(args.data_path)/args.dataset,
            output_csv=output_csv
        )

        print(f"📊 Features saved to {output_csv}")
        print(f"📝 Metadata saved to {output_csv.replace('.csv','_meta.json')}")

    def _interactive_mode(self):
        """Interactive session with same guarantees"""
        print("\n💻 Interactive Mode (type 'exit' to quit)")
        while True:
            cmd = input("(train/predict/inspect)> ").strip().lower()

            if cmd == 'exit':
                break
            elif cmd == 'train':
                self._run_training_interactive()
            elif cmd == 'predict':
                self._run_prediction_interactive()
            elif cmd == 'inspect':
                self._inspect_outputs()
            else:
                print("Available commands: train, predict, inspect, exit")

    def _run_training_interactive(self):
        """Interactive training with same guarantees"""
        print("\nTraining Options:")
        dataset = input(f"Dataset [{self.config['dataset']['name']}]: ")
        dataset = dataset or self.config['dataset']['name']

        trainer = UnifiedTrainingManager()
        model_path = trainer.train_and_save(
            config=self.config,
            train_loader=create_dataloader(f"data/{dataset}/train")
        )
        print(f"\nModel saved to {model_path}")

    def _run_prediction_interactive(self):
        """Interactive prediction with certified path"""
        model_path = input("Model path: ").strip()
        output_csv = input(f"Output CSV [default_{self.config['dataset']['name']}.csv]: ")
        output_csv = output_csv or f"default_{self.config['dataset']['name']}.csv"

        predictor = CertifiedPredictionManager(model_path)
        predictor.generate_standard_features(output_csv=output_csv)
        print(f"Features saved to {output_csv}")
...
STANDARD_COLUMNS = [
    ...
    'cluster_assignment',
    'cluster_confidence'
] + [f'feature_{i}' for i in range(config['model']['feature_dims'])]
